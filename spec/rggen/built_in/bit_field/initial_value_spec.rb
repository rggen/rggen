...
RSpec.describe 'bit_field/initial_value' do
  ...
  def create_bit_field(width, setting_values, *input_value)
  ...
  describe '#initial_value' do
    it '入力された初期値を返す' do
      {
        ...
        initial_values.each do |initial_value|
          bit_field = create_bit_field(width, default_settings, initial_value)
          expect(bit_field).to have_property(:initial_value, initial_value)

          bit_field = create_bit_field(width, default_settings, initial_value.to_f)
          expect(bit_field).to have_property(:initial_value, initial_value)

          bit_field = create_bit_field(width, default_settings, initial_value.to_s)
          expect(bit_field).to have_property(:initial_value, initial_value)

          next if initial_value.negative?

          bit_field = create_bit_field(width, default_settings, format('0x%x', initial_value))
          expect(bit_field).to have_property(:initial_value, initial_value)
        end
      end
    end
    ...
    context '入力が空白の場合' do
      it '既定値 0 を返す' do
        bit_field = create_bit_field(1, default_settings, nil)
        expect(bit_field).to have_property(:initial_value, 0)
  ...
  describe 'initial_value?' do
    it '初期値が設定されたかどうかを返す' do
      ...
      bit_field = create_bit_field(1, default_settings, 1)
    ...
    context '入力が整数に変換できない場合' do
      it 'RegisterMapErrorを起こす' do
        [true, false, 'foo', '0xef_gh', Object.new].each do |value|
          expect {
            create_bit_field(1, default_settings, value)
      ...
      it 'RegisterMapErrorを起こす' do
        {
          ...
          values.each do |value|
            expect {
              create_bit_field(width, default_settings, value)
